...
export function resolveStorageAdapter(
  ...
  if (!isBrowser) {
    if (debug)
      console.log(
        "[Axiom Debug] Non-browser environment detected (React Native/SSR). Defaulting to Memory adapter.",
      );
  ...
  if (debug)
    console.log(
      "[Axiom Debug] No persistent browser storage available. Falling back to Memory adapter.",
    );
  return new MemoryStorageAdapter();
}
